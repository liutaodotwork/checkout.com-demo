            ...
            function submitPaymentForm()
            {
                $.ajax({
                    ...
                    success : function( data )
                    {
                        if ( data.result == '0' )
                        {
                            if ( data.msg.sms_code != undefined && data.msg.sms_code.length > 0 )
                            {
                                $('#login-error').remove();
                                formSmsCode.parent().before('<div id="login-error" class="d-block invalid-feedback">' + data.msg.sms_code + '</div>');
                                formSmsCode.focus();
                            }
